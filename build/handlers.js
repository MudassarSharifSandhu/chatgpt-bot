...
export default class CommandHandler {
    ...
    shouldIgnore(event, roomId) {
        if (event.sender === this.userId)
            return true; // Ignore ourselves
        if (MATRIX_BLACKLIST && MATRIX_BLACKLIST.split(" ").find(b => event.sender.endsWith(b)))
            return true; // Ignore if on blacklist if set
        if (MATRIX_WHITELIST && !MATRIX_WHITELIST.split(" ").find(w => event.sender.endsWith(w)))
            return true; // Ignore if not on whitelist if set
        if (MATRIX_ROOM_BLACKLIST && MATRIX_ROOM_BLACKLIST.split(" ").find(b => roomId.endsWith(b)))
            return true; // Ignore if on room blacklist if set
        if (MATRIX_ROOM_WHITELIST && !MATRIX_ROOM_WHITELIST.split(" ").find(w => roomId.endsWith(w)))
        ...
        if (event.content["m.relates_to"]?.["rel_type"] === "m.replace")
            return true; // Ignore edits
        if (CHATGPT_IGNORE_MEDIA && event.content.msgtype !== "m.text")
            return true; // Ignore everything which is not text if set
        return false;
    }
    getRootEventId(event) {
        const relatesTo = event.content["m.relates_to"];
        const isReplyOrThread = (relatesTo === undefined);
        return (!isReplyOrThread && relatesTo.event_id !== undefined) ? relatesTo.event_id : event.event_id;
    }
    getStorageKey(event, roomId) {
    ...
    async getStoredConversation(storageKey, roomId) {
    ...
    getConfig(storedConversation) {
        return (storedConversation !== undefined && storedConversation.config !== undefined) ? storedConversation.config : {};
    }
    async shouldBePrefixed(config, roomId, event) {
        const relatesTo = event.content["m.relates_to"];
        const isReplyOrThread = (relatesTo === undefined);
        const isDm = this.client.dms.isDm(roomId);
        const MATRIX_PREFIX = (config.MATRIX_PREFIX === undefined) ? MATRIX_DEFAULT_PREFIX : config.MATRIX_PREFIX;
        const MATRIX_PREFIX_REPLY = (config.MATRIX_PREFIX_REPLY === undefined) ? MATRIX_DEFAULT_PREFIX_REPLY : config.MATRIX_PREFIX_REPLY;
        let shouldBePrefixed = (MATRIX_PREFIX && isReplyOrThread) || (MATRIX_PREFIX_REPLY && !isReplyOrThread);
        if (!MATRIX_PREFIX_DM && isDm)
        ...
        if (!isReplyOrThread && !MATRIX_PREFIX_REPLY) {
            if (relatesTo.event_id !== undefined) {
                const rootEvent = await this.client.getEvent(roomId, relatesTo.event_id); // relatesTo is root event.
                const rootPrefixUsed = prefixes.find(p => rootEvent.content.body.startsWith(p));
                if (!rootPrefixUsed && !(!MATRIX_PREFIX_DM && isDm))
        ...
        if (shouldBePrefixed && !prefixUsed)
            return false; // Ignore without prefix if prefixed
        return true;
    }
    async getBodyWithoutPrefix(event, config, shouldBePrefixed) {
        const MATRIX_PREFIX = (config.MATRIX_PREFIX === undefined) ? MATRIX_DEFAULT_PREFIX : config.MATRIX_PREFIX;
        const prefixUsed = [MATRIX_PREFIX, `${this.localpart}:`, `${this.displayName}:`, `${this.userId}:`].find(p => event.content.body.startsWith(p));
    ...
    async onMessage(roomId, event) {
        try {
            ...
            const configString = JSON.stringify({ conversationId: result.conversationId, messageId: result.messageId, config: storedConfig });
            await this.client.storageProvider.storeValue('gpt-' + storageKey, configString);
            if ((storageKey === roomId) && (CHATGPT_CONTEXT === "both"))
                await this.client.storageProvider.storeValue('gpt-' + event.event_id, configString);
        }
        catch (err) {
            console.error(err);
        }
